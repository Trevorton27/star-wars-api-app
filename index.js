...
window.addEventListener('load', async () => {
    ...
    const returnValue = async function () {
        await fetch('https://swapi.dev/api/people/?search=' + inputField.value)
            .then(response => response.json())
            .then((response) => {
                const world = async () => await getHomeWorld(response.homeworld);
                const species =  async () => await getSpecies(response.species);
                const searchedCharacter = response.results;

                const searchReturnData = {
                    name: searchedCharacter.name,
                    height: searchedCharacter.height,
                    mass: searchedCharacter.mass,
                    homeWorld: world,
                    species: species,
                    birthDate: searchedCharacter.birth_year
                }
                console.log('response is: ', searchedCharacter);
                // console.log('search return data is: ', searchReturnData);
    ...
    .addEventListener('click', () => {
        console.log('Im working dawg!');
        returnValue();  
        // console.log(returnValue());      // if (inputField.value !== '') {
